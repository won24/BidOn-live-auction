...
const Discontiuation = () => {
    const [discontiuationList, setDiscontiuationList] = useState([]);
    ...
    return (
        <>
            ...
            <div className="auctionListContainer">
                {(searchItemList.length > 0 ? searchItemList : discontiuationList).map((list) => (
                ...
                {searchItemList.length === 0 && discontiuationList.length === 0 && (
                    <p>해당하는 경매품이 없습니다.</p>
                )}
            </div>
        </>
